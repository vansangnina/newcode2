<?php
	...
	*/
	...
	if($id_order<=0){
		$func->transfer("Không có dữ liệu","/admin/order/man");
		exit();
	}
	...
	if(!$row_detail){
		transfer("Dữ liệu không có thực hoặc đã xóa","/admin/order/man");
		exit();
	}
	...
	//set value company
	$document->setValue('{tencty}', $row_setting["name_vi"]);
	$document->setValue('{diachicty}', $row_setting["address_vi"]);
