...
<div class="control_frm" style="margin-top:25px;">
    <div class="bc">
        <ul id="breadcrumbs" class="breadcrumbs">
        	<li><a href="admin/cate/man_item/<?=$_GET['type']?>"><span>Quản lý <?=$type->title ?></span></a></li>
        	<?php if($_GET['keyword']!=''){ ?>
...
<div class="control_frm" style="margin-top:0;">
  	<div style="float:left;">
    	  <input type="button" class="blueB" value="Thêm" onclick="location.href='admin/cate/add_item/<?=$_GET['type']?>'" />
...
<div class="widget">
  ...
  <table cellpadding="0" cellspacing="0" width="100%" class="sTable withCheck mTable SearchTable" id="checkAll">
    ...
    <tbody>
        ...
        <td align="center">
            <input type="text" value="<?=$value['number']?>" class="tipS smallText update_stt" original-title="Nhập số thứ tự sản phẩm" data-id="<?=$value['id']?>" data-table="<?=$com?>_item" data-type="stt" />
        </td> 
        ...
        <td class="title_name_data">
            <a href="admin/cate/edit_item/<?=$_GET['type']?>?id_list=<?=$value['id_list']?>&id_cat=<?=$value['id_cat']?>&id=<?=$value['id']?>" class="tipS SC_bold"><?=$value['name_vi']?></a>
        </td>
        <?php if(in_array('view',$type->config_open)){ ?> 
        ...
        <?php if(in_array('highlight',$type->config_open)){ ?>
        <td align="center">
        <a data-val2="<?=$_GET['com']?>_item" rel="<?=$value['highlight']?>" data-val3="highlight" class="diamondToggle <?=($value['highlight']==1)?"diamondToggleOff":""?>" data-val0="<?=$value['id']?>" ></a> 
        </td>
        <?php } ?>
      
        <td align="center">
          <a data-val2="<?=$_GET['com']?>_item" rel="<?=$value['shows']?>" data-val3="shows" class="diamondToggle <?=($value['shows']==1)?"diamondToggleOff":""?>" data-val0="<?=$value['id']?>" ></a>   
        </td>
       
        <td class="actBtns">
            <a href="admin/cate/edit_item/<?=$_GET['type']?>?id_list=<?=$value['id_list']?>&id_cat=<?=$value['id_cat']?>&id=<?=$value['id']?>" title="" class="smallButton tipS" original-title="Sửa sản phẩm"><img src="assets/admin/images/icons/dark/pencil.png" alt=""></a>
            <a href="admin/cate/delete_item/<?=$_GET['type']?>?listid=<?=$value['id']?>" onClick="if(!confirm('Xác nhận xóa')) return false;" title="" class="smallButton tipS" original-title="Xóa sản phẩm"><img src="assets/admin/images/icons/dark/close.png" alt=""></a>
        </td>
      </tr>
         <?php } ?>
                </tbody>
