...
<div class="control_frm" style="margin-top:25px;">
    <div class="bc">
        <ul id="breadcrumbs" class="breadcrumbs">
        	<li><a href="admin/product/man/<?=$_GET['type']?>"><span>Quản lý <?=$type->title?></span></a></li>
        	<?php if($_GET['keyword']!=''){ ?>
...
<div class="control_frm" style="margin-top:0;">
  	<div style="float:left;">
    	  <input type="button" class="blueB" value="Thêm" onclick="location.href='admin/product/add/<?=$_GET['type']?>'" />
  ...
  <table cellpadding="0" cellspacing="0" width="100%" class="sTable withCheck mTable SearchTable" id="checkAll">
    <thead>
      <tr>
        ...
        <?php } ?>
        <?php if(in_array('selling',$type->config_open)){ ?>
        <td class="tb_data_small">Bán Chạy</td>
        <?php } ?>
        <?php if(in_array('promotion',$type->config_open)){ ?>
    ...
    <tbody>
        ...
        <?php if(in_array('image',$type->config_open)){ ?>
          <td align="center"><img src="<?=_upload_product_l.$value['thumb']?>" width="60" /></td>
        <?php } ?>
        
        <td class="title_name_data">
            <a href="admin/product/edit/<?=$_GET['type']?>?id=<?=$value['id']?>" class="tipS SC_bold"><?=$value['name_vi']?></a>
        </td>
        <?php if(in_array('view',$type->config_open)){ ?> 
        ...
        <?php if(in_array('selling',$type->config_open)){ ?>
        <td align="center">
        <a data-val2="<?=$_GET['com']?>" rel="<?=$value['selling']?>" data-val3="selling" class="diamondToggle <?=($value['selling']==1)?"diamondToggleOff":""?>" data-val0="<?=$value['id']?>" ></a> 
        </td>
        <?php } ?>
        <?php if(in_array('promotion',$type->config_open)){ ?>
        <td align="center">
        <a data-val2="<?=$_GET['com']?>" rel="<?=$value['promotion']?>" data-val3="promotion" class="diamondToggle <?=($value['promotion']==1)?"diamondToggleOff":""?>" data-val0="<?=$value['id']?>" ></a> 
        </td>
        <?php } ?>
        ...
        <td class="actBtns">
            <a href="admin/product/edit/<?=$_GET['type']?>?id=<?=$value['id']?>" title="" class="smallButton tipS" original-title="Sửa sản phẩm"><img src="assets/admin/images/icons/dark/pencil.png" alt=""></a>
            <a href="admin/product/delete/<?=$_GET['type']?>?listid=<?=$value['id']?>" onClick="if(!confirm('Xác nhận xóa')) return false;" title="" class="smallButton tipS" original-title="Xóa sản phẩm"><img src="assets/admin/images/icons/dark/close.png" alt=""></a>
        </td>
      </tr>
      <?php } ?>
    </tbody>
...
<div class="paging"><?=$this->paging?></div>