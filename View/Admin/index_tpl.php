<?php
    $row_trinhduyet  =  \Library\ClassPDO::query("select browser,count(browser) as dem from table_counter group by browser");
    $row_hdh2  =  \Library\ClassPDO::query("select os,count(os) as dem from table_counter group by os order by os asc");
    $row_thietbi  =  \Library\ClassPDO::query("select device,count(device) as dem from table_counter group by device order by device asc");
    $row_tb  =  \Library\ClassPDO::query("select device,count(device) as dem from table_counter group by device order by device asc");
    $row_quocgia  =  \Library\ClassPDO::query("select nation,count(nation) as dem from table_counter where nation!='' group by nation order by dem desc limit 0,10");
    $row_dentu  =  \Library\ClassPDO::query("select website,count(website) as dem from table_counter where website!='' group by website order by dem desc limit 0,8");
...
<script src="Assets/admin/js/chart/js/chartli.js"></script>
    ...
    $('#container').highcharts({
        ...
        series: [{
            ...
            <?php for($i = 1; $i <= $daysInMonth; $i++):
                ...
                $todayrc  =  \Library\ClassPDO::row("SELECT COUNT(*) AS todayrecord FROM table_counter WHERE tm>='$begin' and tm<'$end'");
...
<script src="Assets/admin/js/highcharts/highcharts.js"></script>
<script src="Assets/admin/js/highcharts/modules/exporting.js"></script>
            ...
            series: [
                {
                    ...
                    data: [
                        { value: <?=$row_hdh2[1]['dem']?>, name: '<?=$row_hdh2[1]['os']?>' },
                        { value: <?=$row_hdh2[2]['dem']?>, name: '<?=$row_hdh2[2]['os']?>' },
                        { value: <?=$row_hdh2[3]['dem']?>, name: '<?=$row_hdh2[3]['os']?>' }
                    ]
                },
                {
                    name: 'Access Sources',
             ...
             <?php for($i=1;$i<count($row_thietbi);$i++){ ?>
                {
                    ...
                    data: [
                        ...
                        { name: '<?=$row_thietbi[$i]['device']?>', value: <?=round(phantram($row_thietbi[$i]['dem'],$tong))?>, itemStyle: labelTop }
                    ]
                },
                <?php } ?>
                {
                    ...
                    data: [
                        ...
                        { name: '<?=$row_thietbi[0]['device']?>', value: <?=round(phantram($row_thietbi[0]['dem'],$tong))?>, itemStyle: labelTop }
                    ]
                }
            ]
        };
