<div id="info">
    <div id="sanpham">
        <div class="thanh_title"><h2><?=$this->model->title_detail?> " <?=$_GET['keywords']?> " </h2> </div> 
        <div class="clear"></div>
        <ul class="khung_sp ul">
            <?php if(count($this->model->data)!=0){?>
                <?php foreach ($this->model->data as $key => $value) {?>
                  <li class="item">
                      <a class="effect-v3 img" href="san-pham/<?=$value['slug']?>.html"><img src="<?=_upload_product_l.'259x230x1/'.$value['photo']?>" alt="<?=$value['ten_'.$lang]?>" /></a>
                      <h3><a href="san-pham/<?=$value['slug']?>.html"><?=$value['name_'.$lang]?></a></h3>
