...
class Product extends ModelAdmin{   
    ...
    { 
    	parent::__construct();
        switch($this->act){
			case "man": $this->data=$this->get_mans(); $this->view = "product/items"; break;
			case "add":	$this->view = "product/item_add"; break;
			case "edit": $this->data=$this->get_man(); $this->view = "product/item_add"; break;
	...
	function get_man(){
	    ...
	    $item  =  ClassPDO::row("select * from #_product where id=:id");
	...
	function save_man(){
		...
		if($photo = Functions::upload_image("file",_img_type,_upload_product_l,$file_name)){
			$data['photo'] = $photo;
			//dongdauanh($photo,_upload_product_l);	
			$data['thumb'] = Functions::create_thumb($data['photo'], _width_thumb, _height_thumb, _upload_product_l,$file_name,_style_thumb);
			if($id){
				ClassPDO::setTable('product');
				ClassPDO::setWhere('id',$id);
				ClassPDO::setType('row');
				$row = ClassPDO::select('photo,thumb');
				if($row){
					Functions::delete_file(_upload_product_l.$row['photo']);	
					Functions::delete_file(_upload_product_l.$row['thumb']);				
				}
			}	
		}
		...
		if (isset($_FILES['files'])) {
            for($i=0;$i<count($_FILES['files']['name']);$i++){
            	if($_FILES['files']['name'][$i]!=''){
					...
					$data['photo'] = $photo;
					//dongdauanh($data['photo'],_upload_product_l);
		...
		for ($i=0 ; $i<count($listid) ; $i++){
			...
			if(count($photo_lq)>0){
				for($j=0;$j<count($photo_lq);$j++){
					Functions::delete_file(_upload_cate.$photo_lq[$j]['photo']);
					Functions::delete_file(_upload_cate.$photo_lq[$j]['thumb']);
				}
			}
			ClassPDO::bindMore(array("id"=>$id,"type"=>$type));
			...
			ClassPDO::bindMore(array("id"=>$id));
			$row = ClassPDO::row("select id,photo,thumb from #_product where id=:id ");
			
			if($row){
				Functions::delete_file(_upload_product_l.$row['photo']);
				Functions::delete_file(_upload_product_l.$row['thumb']);
				ClassPDO::bindMore(array("id"=>$id));
				ClassPDO::query("delete from #_product where id=:id ");
			}
		}
		Functions::redirect($_SESSION['links_re']);
	}
