...
class Link extends ModelAdmin{   

    public function __construct($router)
    { 
    	parent::__construct();
        switch($this->act){
			case "man": $this->data=$this->get_mans(); $this->view = "link/items"; break;
			case "add":	$this->view = "link/item_add"; break;
			case "edit": $this->data=$this->get_man(); $this->view = "link/item_add"; break;
	...
	public function get_man(){
	    ...
	    $item  =  ClassPDO::row("select * from #_link where id=:id");
	    if(!$item) Functions::transfer("Dữ liệu không có thực", $_SESSION['links_re']);
	    return $item;
	}

	public function save_man(){
	...
	public function delete_man(){
		...
		for ($i=0 ; $i<count($listid) ; $i++){
			$id=$listid[$i]; 
			ClassPDO::bindMore(array("id"=>$id));
			$row = ClassPDO::row("select id,photo,thumb from #_link where id=:id ");
			if($row){
				Functions::delete_file(_upload_hinhanh.$row['photo']);
				Functions::delete_file(_upload_hinhanh.$row['thumb']);
				ClassPDO::bindMore(array("id"=>$id));
				ClassPDO::query("delete from #_link where id=:id ");
			}
		}
		Functions::redirect($_SESSION['links_re']);
	}
